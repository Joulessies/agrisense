...
const state = {
  ...
  activity: [
    {
      time: "8:24 AM",
      text: "Soil moisture dropped to 18% in Zone B",
      tone: "warning",
    },
...
try {
  ...
  if (savedInterval)
    state.device.pollIntervalMs = Number(savedInterval) || 5000;
...
function renderAlerts() {
  ...
  Object.values(state.sensors).forEach((s) => {
    ...
    const severity =
      st === "high" && s.value > s.optimal.max * 1.25 ? "critical" : "warning";
    live.push({
      sensorId: s.id,
      sensor: s.label,
      severity,
...
function renderSettings() {
  ...
  const statusPill = (() => {
    if (!dev.enabled)
      return '<span class="text-[11px] text-slate-500 bg-slate-100 px-2 py-1 rounded-full"><i class="fa-solid fa-plug-circle-xmark mr-1"></i>Disconnected (simulation active)</span>';
    if (dev.status === "connected")
      return '<span class="text-[11px] text-agri-700 bg-agri-100 px-2 py-1 rounded-full"><i class="fa-solid fa-plug-circle-check mr-1"></i>Live data flowing</span>';
    if (dev.status === "error")
      return `<span class="text-[11px] text-rose-700 bg-rose-100 px-2 py-1 rounded-full"><i class="fa-solid fa-circle-exclamation mr-1"></i>Error: ${dev.lastError || "connection failed"}</span>`;
  ...
  endpointInput?.addEventListener("input", () => {
    state.device.endpoint = endpointInput.value.trim();
  });
  ...
  $("#deviceDisconnectBtn", container)?.addEventListener("click", () =>
    disconnectDevice(),
  );
  ...
  const persistSupabase = () => {
    try {
      ...
      localStorage.setItem(
        "agrisense.supabase.anonKey",
        state.supabase.anonKey,
      );
      localStorage.setItem(
        "agrisense.supabase.logLimit",
  ...
  $("#supabaseLoadBtn", container)?.addEventListener("click", () =>
    loadSupabaseReadings(),
  );
  $("#supabaseSeedBtn", container)?.addEventListener("click", () =>
    insertSupabaseTestRow(),
  );
  ...
  $("#settingsSave")?.addEventListener("click", () => {
    ...
    $("#settingsStatus").innerHTML =
      '<i class="fa-solid fa-circle-check text-agri-600 mr-1"></i> Saved — statuses recomputed.';
...
function renderKpiGrid() {
  ...
  const kpis = [
    {
      ...
      value: `${avg(data.soil).toFixed(1)}%`,
      delta: trend(data.soil),
      unit: "%",
      invert: true,
    },
    {
      label: "Avg. temperature",
      value: `${avg(data.temp).toFixed(1)}°C`,
      delta: trend(data.temp),
      unit: "°C",
    },
    {
      label: "Avg. humidity",
      value: `${avg(data.humid).toFixed(0)}%`,
      delta: trend(data.humid),
      unit: "%",
    },
    {
      label: "Current soil",
      value: `${formatNumber(state.sensors.soilMoisture.value, 0)}%`,
      delta: 0,
    },
  ];
  ...
  grid.innerHTML = kpis
    .map((k) => {
      let trendHtml =
        '<p class="text-xs text-slate-500 mt-1"><i class="fa-solid fa-minus"></i> Stable</p>';
      if (Math.abs(k.delta) > 0.1) {
        ...
        const color =
          good && Math.abs(k.delta) < 5 ? "text-agri-600" : "text-amber-600";
...
function normalizeGeminiApiKey(key) {
  ...
  if (!k.startsWith("AIza")) {
    throw new Error(
      "Gemini key should start with AIza… (from Google AI Studio).",
    );
  }
  return k;
}
...
function buildAiTelemetryPayload() {
  return {
    ...
    plant: {
      ageDays: state.plant.age,
      harvestTargetDays: state.plant.harvestAge,
    },
...
async function fetchGeminiInsightsWithModel(apiKey, model) {
  ...
  if (parsedOuter.error)
    throw new Error(parsedOuter.error.message || "Gemini error");
  ...
  const arr = Array.isArray(data.insights)
    ? data.insights
    : Array.isArray(data)
      ? data
      : null;
...
async function refreshAiInsights(opts = {}) {
  ...
  if (!apiKey) {
    ...
    state.aiInsights.geminiItems = null;
    if (!silent)
      toast("Add your Google Gemini API key in Settings (Admin)", "warning");
    renderAiInsights();
    return;
  }
  ...
  try {
    ...
    if (/quota|rate.?limit|429/i.test(msg)) {
      msg =
        "Gemini quota exceeded for this model — wait a minute, check billing at Google AI Studio, or use backup analysis below.";
    }
...
const AI_INSIGHT_TONES = {
  ...
  1: {
    ...
    bg: "bg-sky-50",
    title: "text-sky-900",
    detail: "text-sky-800/90",
    iconBg: "bg-sky-100",
    icon: "text-sky-600",
  },
  0: {
    border: "border-agri-200",
    bg: "bg-agri-50/80",
...
function buildHeuristicInsightItems() {
  ...
  const problemSensors = Object.values(state.sensors).filter(
    (s) => getStatus(s) !== "ok",
  );
  if (problemSensors.length === 0) {
    add(
      0,
      "Environmental balance looks good",
      "All four sensor channels sit within your optimal ranges. Keep irrigation and climate routines steady unless conditions change.",
      "fa-circle-check",
    );
  } else {
    problemSensors.forEach((s) => {
      ...
      if (st === "low") {
        add(
          2,
          `${s.label} is below target`,
          `Reading ${v}${u} vs your ${s.optimal.min}–${s.optimal.max} band — prioritize bringing this back into range.`,
          s.icon,
        );
      } else {
        add(
          2,
          `${s.label} is above target`,
          `Reading ${v}${u} vs ${s.optimal.min}–${s.optimal.max} — intervene before it stays elevated.`,
          s.icon,
        );
      }
    });
  ...
  if (getStatus(sm) === "low" && getStatus(temp) === "high") {
    add(
      3,
      "Water stress risk",
      "Dry soil with elevated heat increases vapor-pressure deficit — leaves lose water quickly. Increase irrigation or cool the canopy.",
  ...
  if (getStatus(temp) === "high" && getStatus(hum) === "low") {
    add(
      ...
      "Hot, dry air pulls moisture from foliage. Pair soil care with humidity recovery (misting, wet pads) when appropriate.",
      "fa-leaf",
    );
  }
  ...
  if (age < harvestAge * 0.85) {
    add(
      0,
      "Growth phase",
      `Crop age ~${age} days (harvest target ${harvestAge}+). Keep monitoring until the maturity window.`,
      "fa-seedling",
    );
    ...
    add(
      1,
      "Harvest window",
      "Estimated maturity reached — confirm with leaf quality before scheduling harvest.",
      "fa-wheat-awn",
    );
  }
  ...
  if (state.device.enabled) {
    if (state.device.status === "connected") {
      add(
        0,
        "Data source: live device",
        "Insights use real telemetry from your gateway when values update — not pure simulation.",
        "fa-wifi",
      );
      ...
      add(
        ...
        `Values may be stale or simulated. Last error: ${state.device.lastError || "unknown"}.`,
  ...
  if (rows.length >= 4) {
    ...
    if (soilVals.length >= 4) {
      ...
      if (d <= -4) {
        add(
          2,
          "Cloud trend: soil declining",
          `Last ${soilVals.length} Supabase samples show moisture down ~${Math.abs(d)} pts — compare with live soil on the dashboard.`,
          "fa-cloud",
        );
      } else if (d >= 4) {
        add(
          0,
          "Cloud trend: soil recovering",
          "Recent stored samples show moisture trending up — your adjustments may be taking effect.",
          "fa-cloud",
        );
      }
    }
    ...
    if (temps.length >= 3) {
      ...
      if (tmax - tmin > 4) {
        add(
          ...
          `${tmax.toFixed(1)}–${tmin.toFixed(1)}°C range in recent cloud logs — review heating, vents, or shade timing.`,
          "fa-temperature-half",
        );
      }
    }
  } else if (
    state.supabase.projectUrl &&
    state.supabase.anonKey &&
    rows.length === 0 &&
    !state.supabase.loading
  ) {
    add(
      1,
      "No cloud history loaded",
      "Open Settings → Supabase cloud log and click Load to unlock trend-based insights.",
      "fa-database",
    );
  }
  ...
  if (offline.length > 0) {
    add(
      1,
      "Coverage gap",
      `${offline.length} node(s) offline (${offline.map((n) => n.id).join(", ")}) — stress in those zones may not show here.`,
      "fa-tower-broadcast",
    );
  }
...
function renderAiInsights() {
  ...
  if (apiKey && state.aiInsights.geminiItems?.length > 0) {
    ...
    metaLine = [
      ...
      state.aiInsights.updatedAt
        ? `Updated ${formatTime(state.aiInsights.updatedAt)}`
        : "",
    ...
    apiKey &&
    !state.aiInsights.geminiItems?.length &&
    !state.aiInsights.error
  ) {
    items = [];
    metaLine =
      "Gemini API key set — click **Refresh analysis** to generate AI insights, or open this tab again to auto-run once.";
    list.innerHTML = `
    ...
    items = [];
    metaLine =
      "Add a free Google Gemini API key under Settings → AI insights (Gemini) to enable real AI.";
    list.innerHTML = `
...
function renderHeader() {
  ...
  const isAdmin = state.role === "admin";
  $("#userRoleLabel").textContent = isAdmin
    ? "System Administrator"
    : "Field Operator";
  $("#greetingHeader").textContent =
    `${timeBasedGreeting()}, ${name.split(" ")[0]}`;
...
function renderRoleBanner() {
  if (state.role === "admin") {
    ...
    if (nodeCount)
      nodeCount.textContent = state.nodes.filter((n) => n.online).length;
  } else {
    const plantAge = $("#farmerPlantAge");
    const activeNodes = $("#farmerActiveNodes");
    if (plantAge) plantAge.textContent = state.plant.age;
    if (activeNodes)
      activeNodes.textContent = state.nodes.filter((n) => n.online).length;
  }
}
...
function renderAll() {
  const safe = (fn) => {
    try {
      ...
      console.error("[renderAll]", fn.name, e);
    }
  };
...
async function pollDevice() {
  ...
  try {
    ...
    if (state.device.consecutiveErrors === 4) {
      ...
      addActivity(
        `Device connection issues: ${state.device.lastError}`,
        "warning",
      );
    }
  }
...
function disconnectDevice(silent = false) {
  ...
  if (!silent) {
    addActivity(
      "Disconnected from device — falling back to simulation",
      "info",
    );
...
// ==========================================================================
// ============== SUPABASE CLIENT & REAL-TIME WEB_SOCKET HUB ================
// ==========================================================================

function initSupabaseClient() {
  if (state.supabase.channel) {
    state.supabase.channel.unsubscribe();
    state.supabase.channel = null;
  }
  state.supabase.client = null;
  state.supabase.realtimeStatus = "disconnected";
  renderRealtimeIndicator();

  const url = (state.supabase.projectUrl || "").trim();
  const key = (state.supabase.anonKey || "").trim();

  if (!url || !key) return;

  try {
    const cleanUrl = url.replace(/\/$/, "");
    const cleanKey = normalizeSupabaseAnonKey(key);

    if (typeof supabase !== "undefined" && supabase.createClient) {
      state.supabase.realtimeStatus = "connecting";
      renderRealtimeIndicator();
      ...
      state.supabase.channel = state.supabase.client
        ...
        .on(
          ...
          (payload) => {
            ...
            handleIncomingRealtimeReading(payload.new);
          }
        )
        .subscribe((status, err) => {
          console.log("WebSocket status:", status, err);
          if (status === "SUBSCRIBED") {
            state.supabase.realtimeStatus = "connected";
...
function renderRealtimeIndicator() {
  const container = $("#realtimeIndicator");
  const pulse = $("#realtimeDotPulse");
  const solid = $("#realtimeDotSolid");
  const label = $("#realtimeLabel");
  const separator = $("#realtimeSeparator");

  if (!container || !pulse || !solid || !label) return;
  ...
  const colors = ["bg-slate-400", "bg-amber-400", "bg-emerald-500", "bg-rose-500"];
  pulse.classList.remove(...colors, "live-dot");
  solid.classList.remove(...colors);

  let text, color, animate;
  switch (state.supabase.realtimeStatus) {
    case "connected":
      text = "Realtime Live";
...
function handleIncomingRealtimeReading(row) {
  if (!row) return;

  if (state.supabase.rows.some((r) => r.id === row.id)) return;

  state.supabase.rows.unshift(row);
  const lim = state.supabase.logLimit || 50;
  if (state.supabase.rows.length > lim) {
    state.supabase.rows.length = lim;
  }

  addActivity(`[Real-Time] Received row #${row.id} from ${row.device_id || "device"}`, "ok");
...
// ============== DATABASE HUB VIEW ROUTER & CONTROLS ==============

let dbCurrentPage = 1;
const dbRowsPerPage = 10;
let dbSearchQuery = "";
...
function renderDatabaseView() {
  ...
  const sb = state.supabase;
  const url = (sb.projectUrl || "").trim();
  const key = (sb.anonKey || "").trim();
  const isAdmin = state.role === "admin";

  if (!url || !key) {
    container.innerHTML = `
      <div class="bg-white rounded-xl border border-slate-200 shadow-card p-10 text-center max-w-lg mx-auto">
        <div class="w-14 h-14 rounded-2xl bg-amber-50 mx-auto flex items-center justify-center mb-4 border border-amber-100">
          <i class="fa-solid fa-database text-amber-500 text-xl"></i>
        </div>
        <h3 class="text-lg font-semibold text-slate-900">Database connection required</h3>
        <p class="text-sm text-slate-500 mt-1">Please configure your Supabase Project URL and Anon Key under the <strong>Settings</strong> panel first to activate the Database Hub.</p>
        <button onclick="setActiveView('settings')" class="mt-4 px-4 py-2 bg-agri-600 hover:bg-agri-700 text-white rounded-xl text-sm font-semibold transition">
          Go to Settings
        </button>
      </div>
    `;
  ...
  const totalRecords = sb.rows.length;
  let timeRange = "No records";
  let avgMoisture = "—";
  let avgTemp = "—";
  let avgHumidity = "—";

  if (totalRecords > 0) {
    const sorted = [...sb.rows].sort((a, b) => new Date(a.recorded_at) - new Date(b.recorded_at));
    const earliest = new Date(sorted[0].recorded_at).toLocaleDateString();
    const latest = new Date(sorted[sorted.length - 1].recorded_at).toLocaleDateString();
    timeRange = earliest === latest ? earliest : `${earliest} – ${latest}`;

    const validMoisture = sb.rows.map(r => r.soil_moisture).filter(v => typeof v === 'number' && !isNaN(v));
  ...
  if (dbFilterSource !== "all") {
    filteredRows = filteredRows.filter(r => {
      ...
      if (dbFilterSource === "simulator") return devId.includes("simulator") || devId === "";
      if (dbFilterSource === "esp32") return devId.includes("esp32") || devId.includes("node");
      if (dbFilterSource === "web-test") return devId.includes("test") || devId.includes("web") || devId.includes("operator");
      return true;
    });
  }
  ...
  if (dbSearchQuery) {
    ...
    filteredRows = filteredRows.filter(r =>
      ...
      (r.temperature != null && String(r.temperature).includes(q))
    );
  }
  ...
  const rowsHtml = paginatedRows.length === 0
    ? `<tr><td colspan="7" class="px-4 py-8 text-center text-sm text-slate-400">No records found. Click Load or Seed mock data.</td></tr>`
    : paginatedRows.map(r => `
        <tr class="border-b border-slate-100 hover:bg-slate-50/60 text-sm transition-colors duration-150">
        ...
        : (r.device_id || "").toLowerCase().includes("test") || (r.device_id || "").toLowerCase().includes("operator")
          ...
          <td class="px-4 py-3 text-slate-800">${r.humidity != null ? Number(r.humidity).toFixed(1) : "—"}%</td>
          <td class="px-4 py-3 text-slate-800">${r.lux != null ? Math.round(r.lux).toLocaleString() : "—"} lux</td>
        </tr>
      `).join("");

  const wsStatusColor = state.supabase.realtimeStatus === "connected" ? "bg-emerald-500" : state.supabase.realtimeStatus === "connecting" ? "bg-amber-400" : "bg-rose-500";
  const wsStatusText = state.supabase.realtimeStatus === "connected" ? "WebSocket Active" : state.supabase.realtimeStatus === "connecting" ? "WebSocket Connecting…" : "WebSocket Offline";

  container.innerHTML = `
    <div class="grid grid-cols-1 md:grid-cols-4 gap-5">
      <div class="bg-white rounded-xl border border-slate-200 shadow-card p-5">
        <div class="flex items-center justify-between mb-2">
          <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider">Sync Diagnostics</p>
          <span class="w-2.5 h-2.5 rounded-full ${wsStatusColor} flex items-center justify-center relative">
            ${state.supabase.realtimeStatus === "connected" ? '<span class="absolute w-full h-full rounded-full bg-emerald-400 animate-ping"></span>' : ''}
          </span>
        </div>
        <p class="text-base font-bold text-slate-900 leading-tight">${wsStatusText}</p>
        <p class="text-[11px] text-slate-500 mt-1">Real-time inserts automatically update client view.</p>
      </div>

      <div class="bg-white rounded-xl border border-slate-200 shadow-card p-5">
        <div class="flex items-center justify-between mb-2">
          <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider">Total Readings</p>
          <i class="fa-solid fa-server text-slate-300"></i>
        </div>
        <p class="text-2xl font-bold text-slate-900">${totalRecords} row${totalRecords !== 1 ? 's' : ''}</p>
        <p class="text-[11px] text-slate-500 mt-1">${timeRange}</p>
      </div>

      <div class="bg-white rounded-xl border border-slate-200 shadow-card p-5">
        <div class="flex items-center justify-between mb-2">
          <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider">Average Soil Moisture</p>
          <i class="fa-solid fa-droplet text-slate-300"></i>
        </div>
        <p class="text-2xl font-bold text-agri-600">${avgMoisture}</p>
        <p class="text-[11px] text-slate-500 mt-1">Target range: 20% - 40%</p>
      </div>

      <div class="bg-white rounded-xl border border-slate-200 shadow-card p-5">
        <div class="flex items-center justify-between mb-2">
          <p class="text-xs text-slate-500 font-semibold uppercase tracking-wider">Average Climate</p>
          <i class="fa-solid fa-temperature-half text-slate-300"></i>
        </div>
        <p class="text-base font-bold text-slate-900">Temp: <span class="text-amber-600">${avgTemp}</span></p>
        <p class="text-xs text-slate-500 mt-0.5">Humidity: <span class="text-sky-500">${avgHumidity}</span></p>
      </div>
    </div>

    <div class="bg-white rounded-xl border border-slate-200 shadow-card p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex flex-wrap items-center gap-2">
        <button id="dbLoadBtn" class="px-4 py-2 bg-agri-600 hover:bg-agri-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition">
          <i class="fa-solid fa-arrow-rotate-right text-[10px]"></i> Load from Cloud
        </button>
        <button id="dbSeedBtn" ${isAdmin ? "" : "disabled"} class="admin-only px-4 py-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition disabled:opacity-50">
          <i class="fa-solid fa-flask text-[10px] text-agri-600"></i> Seed 7-Day History
        </button>
        <button id="dbClearBtn" ${isAdmin ? "" : "disabled"} class="admin-only px-4 py-2 border border-rose-100 bg-white hover:bg-rose-50/50 text-rose-600 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition disabled:opacity-50">
          <i class="fa-solid fa-trash-can text-[10px]"></i> Wipe Table
        </button>
      </div>
      <div class="flex items-center gap-2">
        <span class="text-xs text-slate-400">Export:</span>
        <button id="dbExportCSV" class="px-3.5 py-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition">
          <i class="fa-solid fa-file-csv text-[11px] text-emerald-600"></i> CSV
        </button>
        <button id="dbExportJSON" class="px-3.5 py-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition">
          <i class="fa-solid fa-file-code text-[11px] text-indigo-600"></i> JSON
        </button>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-card lg:col-span-8 flex flex-col justify-between overflow-hidden min-h-[500px]">
        <div>
          <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <h3 class="text-sm font-semibold text-slate-800">Supabase Records</h3>
            
            <div class="flex items-center gap-2">
              <select id="dbFilterSelect" class="text-xs border border-slate-200 rounded-lg px-2.5 py-1.5 bg-white text-slate-600 font-medium">
                <option value="all" ${dbFilterSource === "all" ? "selected" : ""}>All Sources</option>
                <option value="simulator" ${dbFilterSource === "simulator" ? "selected" : ""}>Simulators</option>
                <option value="esp32" ${dbFilterSource === "esp32" ? "selected" : ""}>ESP32 Nodes</option>
                <option value="web-test" ${dbFilterSource === "web-test" ? "selected" : ""}>Operator Input</option>
              </select>
              
              <div class="relative text-slate-400">
                <i class="fa-solid fa-magnifying-glass absolute left-3 top-2.5 text-[10px]"></i>
                <input id="dbSearchInput" type="text" placeholder="Search logs..." value="${dbSearchQuery}"
                  class="pl-7 pr-3 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-agri-500/20 w-44 bg-white text-slate-800" />
              </div>
            </div>
          </div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs">
              <thead class="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-500 font-semibold border-b border-slate-100">
                <tr>
                  <th class="px-4 py-3">ID</th>
                  <th class="px-4 py-3">Timestamp</th>
                  <th class="px-4 py-3">Device / Origin</th>
                  <th class="px-4 py-3">Soil Moisture</th>
        ...
        <div class="px-5 py-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 bg-slate-50/50">
          <span>Showing <strong class="text-slate-700">${totalFiltered === 0 ? 0 : startIdx + 1} – ${Math.min(totalFiltered, startIdx + dbRowsPerPage)}</strong> of <strong class="text-slate-700">${totalFiltered}</strong> readings</span>
          
          <div class="flex items-center gap-1">
            <button id="dbPrevPage" ${dbCurrentPage === 1 ? "disabled" : ""}
              class="px-2.5 py-1.5 border border-slate-200 rounded hover:bg-slate-50 font-medium transition disabled:opacity-50 disabled:hover:bg-transparent bg-white">
              <i class="fa-solid fa-chevron-left text-[9px]"></i> Prev
            </button>
            <span class="px-3 py-1 font-semibold text-slate-700">Page ${dbCurrentPage} / ${totalPages}</span>
            <button id="dbNextPage" ${dbCurrentPage === totalPages ? "disabled" : ""}
              class="px-2.5 py-1.5 border border-slate-200 rounded hover:bg-slate-50 font-medium transition disabled:opacity-50 disabled:hover:bg-transparent bg-white">
      ...
      <div class="bg-white rounded-xl border border-slate-200 shadow-card p-5 lg:col-span-4 h-fit">
        <h3 class="text-sm font-semibold text-slate-800 border-b border-slate-100 pb-3 mb-4 flex items-center gap-2">
          <i class="fa-solid fa-circle-plus text-agri-600"></i> Log Custom Sample
        </h3>

        <form id="dbManualForm" class="space-y-4 text-xs">
          <div>
            <label for="mFormDeviceId" class="block text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">Device ID / Origin</label>
            <input id="mFormDeviceId" type="text" required placeholder="e.g. ESP32-ZONE-A" value="Operator-Manual"
              class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-agri-500/20 bg-white text-slate-800" />
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div>
              <label for="mFormMoisture" class="block text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">Soil Moisture (%)</label>
              <input id="mFormMoisture" type="number" required min="0" max="100" placeholder="e.g. 24" value="28"
                class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-agri-500/20 bg-white text-slate-800" />
            </div>
            <div>
              <label for="mFormTemp" class="block text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">Temperature (°C)</label>
              <input id="mFormTemp" type="number" step="0.1" required min="-10" max="60" placeholder="e.g. 28.5" value="29.0"
                class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-agri-500/20 bg-white text-slate-800" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label for="mFormHumidity" class="block text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">Humidity (%)</label>
              <input id="mFormHumidity" type="number" step="0.1" required min="0" max="100" placeholder="e.g. 62.0" value="65.0"
                class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-agri-500/20 bg-white text-slate-800" />
            </div>
            <div>
              <label for="mFormLux" class="block text-[10px] text-slate-500 uppercase tracking-wide font-bold mb-1">Light Intensity (Lux)</label>
              <input id="mFormLux" type="number" required min="0" max="100000" placeholder="e.g. 15000" value="14200"
                class="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-agri-500/20 bg-white text-slate-800" />
            </div>
          </div>

          <button type="submit" class="w-full py-2.5 bg-agri-600 hover:bg-agri-700 text-white rounded-lg font-semibold flex items-center justify-center gap-1.5 transition">
            <i class="fa-solid fa-paper-plane text-[10px]"></i> Insert into Supabase
          </button>
        </form>
      </div>
  ...
  $("#dbSeedBtn")?.addEventListener("click", () => {
    seedSupabaseHistoricalData();
  });

  $("#dbClearBtn")?.addEventListener("click", () => {
    if (confirm("Are you absolutely sure you want to delete all sensor reading logs from the cloud database? This action is irreversible.")) {
      clearSupabaseTable();
    }
  });

  $("#dbExportCSV")?.addEventListener("click", () => {
    exportSupabaseCSV();
  });

  $("#dbExportJSON")?.addEventListener("click", () => {
    exportSupabaseJSON();
  });
  ...
  $("#dbFilterSelect")?.addEventListener("change", (e) => {
    dbFilterSource = e.target.value;
    dbCurrentPage = 1;
    renderDatabaseView();
  });
  ...
  $("#dbManualForm")?.addEventListener("submit", async (e) => {
    e.preventDefault();
    await submitManualFormReading();
  });

  renderRoleVisibility();
}

async function seedSupabaseHistoricalData(daysCount = 7) {
  if (!state.supabase.client) {
    toast("Supabase client not initialized", "warning");
    return;
  }

  const btn = $("#dbSeedBtn");
  if (btn) btn.disabled = true;
  toast("Generating and uploading historical data...", "info");
  ...
  for (let i = totalPoints; i >= 0; i--) {
    ...
    const tempBase = 26;
    const tempVariance = 4 * Math.sin(((hour - 8) / 24) * 2 * Math.PI);
    const temperature = Number((tempBase + tempVariance + (Math.random() - 0.5) * 0.8).toFixed(1));

    const humidBase = 60;
    const humidVariance = -10 * Math.sin(((hour - 8) / 24) * 2 * Math.PI);
    const humidity = Number((humidBase + humidVariance + (Math.random() - 0.5) * 1.5).toFixed(1));

    let lux = 0;
    if (hour >= 6 && hour <= 18) {
      lux = Math.round(18000 * Math.sin(((hour - 6) / 12) * Math.PI) + Math.random() * 1000);
    }

    let soil_moisture = 30 - ((i % 48) * 0.4) + (Math.random() - 0.5) * 0.5;
    if (soil_moisture < 16) soil_moisture = 32 + (Math.random() - 0.5) * 2;
    soil_moisture = Math.round(clamp(soil_moisture, 10, 45));

    readings.push({
      recorded_at: time.toISOString(),
      device_id: "Simulator-Historical",
      soil_moisture,
      temperature,
      humidity,
      lux
  ...
  try {
    ...
    toast(`Successfully seeded ${readings.length} history readings!`, "success");
    addActivity(`Seeded ${readings.length} historical readings in Supabase`, "ok");
...
async function clearSupabaseTable() {
  ...
  try {
    ...
    toast("All database logs cleared!", "success");
    addActivity("Cleared all sensor readings from cloud database", "warning");
    await loadSupabaseReadings();
  } catch (err) {
    console.error(err);
    toast(`Failed to clear table: ${err.message}`, "error");
  } finally {
    if (btn) btn.disabled = false;
  }
}

async function submitManualFormReading() {
  if (!state.supabase.client) return;

  const devId = $("#mFormDeviceId").value.trim() || "Manual-Entry";
  const soil = Math.round($("#mFormMoisture").value);
  const temp = Number($("#mFormTemp").value);
  const hum = Number($("#mFormHumidity").value);
  const lx = Math.round($("#mFormLux").value);
...
function exportSupabaseCSV() {
  ...
  const headers = ["ID", "Recorded At", "Device ID", "Soil Moisture (%)", "Temperature (°C)", "Humidity (%)", "Light (Lux)"];
  ...
  link.setAttribute("download", `agrisense-cloud-database-${new Date().toISOString().split("T")[0]}.csv`);
  ...
  addActivity("CSV database export generated", "info");
}

function exportSupabaseJSON() {
  const rows = state.supabase.rows;
  if (!rows.length) {
    toast("No logs to export", "warning");
    return;
  ...
  const link = document.createElement("a");
  link.setAttribute("href", url);
  link.setAttribute("download", `agrisense-cloud-database-${new Date().toISOString().split("T")[0]}.json`);
  document.body.appendChild(link);
  link.click();
  document.body.removeChild(link);
  URL.revokeObjectURL(url);
  ...
  addActivity("JSON database export generated", "info");
}
...
async function loadSupabaseReadings() {
  ...
  try {
    ...
    if (state.supabase.client) {
      const { data: dbData, error } = await state.supabase.client
        .from("sensor_readings")
        .select("*")
    ...
    addActivity(
      `Loaded ${state.supabase.rows.length} cloud reading(s) from Supabase`,
      "info",
    );
    if (applied > 0) {
      addActivity(
        `Dashboard updated from latest cloud sample (${applied} sensor(s))`,
        "ok",
      );
    }
    toast(
      `Loaded ${state.supabase.rows.length} row(s)${applied ? " · dashboard synced" : ""}`,
      "success",
    );
    if (applied > 0) {
      renderSensorCards();
    ...
    state.supabase.rows = [];
    toast(
      `Supabase: ${formatSupabaseError(state.supabase.lastError)}`,
      "error",
    );
  } finally {
...
async function insertSupabaseTestRow() {
  ...
  try {
    ...
    state.supabase.lastError = err.message || String(err);
    toast(
      `Supabase insert: ${formatSupabaseError(state.supabase.lastError)}`,
      "error",
    );
  } finally {
    state.supabase.loading = false;
...
const viewMeta = {
  dashboard: {
    title: "Dashboard",
    subtitle: "Real-time overview of your aloe vera crop",
  },
  sensors: {
    title: "Sensors",
    subtitle: "Diagnostics for all connected nodes",
  },
  alerts: { title: "Alerts", subtitle: "Triggered warnings and notifications" },
  analytics: {
    title: "Analytics",
    subtitle: "Insights and trends across your crop's vital signs",
  },
  database: {
    title: "Database Hub",
    subtitle: "Real-time cloud database diagnostics and data management",
  },
  settings: {
    title: "Settings",
    subtitle: "Configure thresholds and preferences",
  },
  users: {
    title: "User Management",
    subtitle: "Manage credentials, names, and system roles",
  },
};
  ...
  radarChart = new Chart(canvas.getContext("2d"), {
    ...
    options: {
      ...
      plugins: {
        legend: {
          position: "bottom",
          labels: { font: { size: 11 }, color: "#475569", boxWidth: 12 },
        },
      },
      scales: {
        r: {
...
function getBarData(range) {
  ...
  const readings = rows
    .map(r => ({
      ...
      soil: Number(r.soil_moisture ?? 0),
      temp: Number(r.temperature ?? 0),
      humid: Number(r.humidity ?? 0),
    }))
    .filter(r => !isNaN(r.recorded_at.getTime()))
  ...
  if (range === "30d") {
    ...
    readings.forEach(r => {
      ...
      if (diffDays <= 30) {
        ...
        weeks[3 - weekIdx].push(r);
      }
    });

    const labels = ["Wk 1", "Wk 2", "Wk 3", "Wk 4"];
    const soil = weeks.map(w => w.length ? Math.round(avg(w.map(x => x.soil))) : 20);
    const temp = weeks.map(w => w.length ? Number(avg(w.map(x => x.temp)).toFixed(1)) : 27);
    const humid = weeks.map(w => w.length ? Math.round(avg(w.map(x => x.humid))) : 60);

    return { labels, soil, temp, humid };
  }
  ...
  if (range === "season") {
    ...
    const activeMonths = Object.keys(months).slice(-3);
    if (activeMonths.length < 3) {
      activeMonths.unshift(...["Mar", "Apr", "May"].filter(m => !activeMonths.includes(m)));
    }
    const finalMonths = activeMonths.slice(-3);

    const labels = finalMonths;
    const soil = finalMonths.map(m => months[m]?.length ? Math.round(avg(months[m].map(x => x.soil))) : 25);
    const temp = finalMonths.map(m => months[m]?.length ? Number(avg(months[m].map(x => x.temp)).toFixed(1)) : 28);
    const humid = finalMonths.map(m => months[m]?.length ? Math.round(avg(months[m].map(x => x.humid))) : 62);
  ...
  const last7Days = [];
  for (let i = 6; i >= 0; i--) {
    last7Days.push(new Date(now.getTime() - i * 24 * 60 * 60 * 1000));
  }

  const labels = last7Days.map(d => dayNames[d.getDay()]);
  const dayGroups = last7Days.map(() => []);

  readings.forEach(r => {
    last7Days.forEach((d, idx) => {
      if (r.recorded_at.toDateString() === d.toDateString()) {
        dayGroups[idx].push(r);
      }
    });
  });

  const soil = dayGroups.map((g, idx) => g.length ? Math.round(avg(g.map(x => x.soil))) : 20 + idx * 2);
  const temp = dayGroups.map((g, idx) => g.length ? Number(avg(g.map(x => x.temp)).toFixed(1)) : 27 + (idx % 2));
  const humid = dayGroups.map((g, idx) => g.length ? Math.round(avg(g.map(x => x.humid))) : 60 + idx);
...
function renderUsersView() {
  ...
  state.auth.users.forEach((u) => {
    const isSelf =
      state.auth.currentUser && state.auth.currentUser.username === u.username;
...
function openUserModal(mode = "add", username = "") {
  ...
  if (mode === "add") {
    ...
    $("#userFormPasswordLabel").textContent =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("#registerForm")?.addEventListener("submit", (e) => {
    ...
    if (name.length < 2) {
      showRegErr("Please enter your full name.");
      return;
    }
    if (username.length < 3) {
      showRegErr("Username must be at least 3 characters.");
      return;
    }
    ...
    if (state.auth.users.some((u) => u.username === username)) {
      showRegErr("That username is already taken. Choose another.");
      return;
    }
  ...
  $("#analyticsRange")?.addEventListener("change", (e) => {
    ...
    updateAnalyticsCharts();
    addActivity(
      `Analytics range set to ${e.target.selectedOptions[0].textContent}`,
      "info",
    );
  });
